...
# SSH deployment script - connects to 134.199.237.34 and runs deployment
# This allows automated deployment from Replit to your new Ubuntu server
...
upload_files() {
    ...
    cp backup-new-server.sh deploy-package/
    cp deploy-to-new-server.sh deploy-package/
...
backup_current() {
    ...
    run_remote "cd $TEMP_DIR && chmod +x backup-new-server.sh && ./backup-new-server.sh"
...
deploy_new() {
    ...
    run_remote "cd $TEMP_DIR && chmod +x deploy-to-new-server.sh && ./deploy-to-new-server.sh"
...
main() {
    ...
    if verify_deployment; then
        ...
        echo "🔧 Admin: admin / admin123 (Full system administration)"
        echo "👔 Employee: employee / employee123 (Job posting management)"  
        echo "👤 Customer: customer1, customer2, customer3 / customer123 (Job applications)"
    else
        echo ""
