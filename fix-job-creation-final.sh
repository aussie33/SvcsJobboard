...
echo "Fixing job creation button functionality..."

cd /var/www/career-portal

# Create a fully working server with proper job creation
cat > server/complete-server.js << 'EOF'
...
# Test categories
CATEGORIES_RESPONSE=$(curl -b /tmp/session.txt -s http://localhost:8080/api/categories)
echo "Categories available: $(echo $CATEGORIES_RESPONSE | jq -r 'length') categories"
...
JOB_DATA='{
  "job": {
    ...
    "categoryId": 1,
    "shortDescription": "Join our team as a Senior Software Engineer",
    "fullDescription": "We are looking for a skilled Senior Software Engineer to join our dynamic team.",
...
JOBS_COUNT=$(curl -b /tmp/session.txt -s http://localhost:8080/api/jobs | jq -r 'length')
