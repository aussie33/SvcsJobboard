# Deploy Your Working Replit Version to DigitalOcean

Since the deployment requires your server password, here are the exact commands to run manually:
...
app.post('/api/jobs', async (req, res) => {
  try {
    ...
    res.status(201).json({ ...mapJob(result.rows[0]), tags: [] });
  } catch (error) {
...
```bash
scp career-portal-deploy.tar.gz root@64.225.6.33:/tmp/
```
...
## Access Your Career Portal
After deployment: http://64.225.6.33
...
This deploys the exact working version from your Replit environment with all the authentication fixes and job creation functionality.