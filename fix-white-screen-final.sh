...
echo "Creating enhanced server with better static file handling..."

cd /var/www/career-portal

# Create enhanced server with proper static file serving
...
app.listen(PORT, '0.0.0.0', () => {
  console.log('=== Enhanced Career Portal Server Started ===');
  console.log(`Port: ${PORT}`);
  console.log(`URL: http://64.225.6.33:${PORT}`);
...
# Stop current server and start enhanced version
pkill -f complete-server.js

# Start enhanced server
PORT=8080 nohup node server/enhanced-server.js > /var/log/career-portal.log 2>&1 &
...
curl -I http://localhost:8080/assets/index-BiDWT6-L.js
...
JOB_RESULT=$(curl -b /tmp/session.txt \
  -X POST http://localhost:8080/api/jobs \
  -H "Content-Type: application/json" \
  -d '{"job":{"title":"Enhanced Server Test","department":"Engineering","categoryId":1,"shortDescription":"Testing enhanced server","requirements":"Frontend functionality","type":"full-time","location":"Remote","status":"active"}}' \
  -s)

echo "Job creation: $JOB_RESULT"

rm -f /tmp/session.txt

echo ""
echo "=== Enhanced Server Deployed ==="
echo "Access: http://64.225.6.33:8080"
